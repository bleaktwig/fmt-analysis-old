...
public class Data {
     ...
     * @param rn        : Number of ranges to try.
     * @param pltRanArr : Array of plot ranges.
     * @return
     */
    public static DataGroup[] createDataGroups(int ln, int zn, int rn,
            int[] pltRanArr) {
        DataGroup[] dgFMT = new DataGroup[zn*rn];

        for (int zi=0; zi<zn; ++zi) {
            for (int ri=0; ri<rn; ++ri) {
                dgFMT[zi*rn + ri] = new DataGroup(3,2);
                for (int li=1; li<=ln; ++li) {
                    double r = pltRanArr[ri];
                    ...
                    dgFMT[zi*rn + ri].addDataSet(hi_cluster_res, li-1);
                    dgFMT[zi*rn + ri].addDataSet(f1_res, li-1);
                    dgFMT[zi*rn + ri].addDataSet(hi_cluster_res_strip, li-1+ln);
                }
            }
        }
